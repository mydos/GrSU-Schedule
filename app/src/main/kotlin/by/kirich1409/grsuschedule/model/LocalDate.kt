...
class LocalDate : Parcelable, Comparable<LocalDate> {
    ...
    private val shortDateFormat: DateFormat
            by lazy { SimpleDateFormat("EE, d MMM", LOCALE_RU) }

    private val longDateFormat: DateFormat
            by lazy { SimpleDateFormat("EEEE, d MMMM", LOCALE_RU) }
    ...
    public fun format(short: Boolean, upperCaseFirstLetter: Boolean = false): String {
        val dateFormat = if (short) shortDateFormat else longDateFormat
        val dateFormatted = dateFormat.format(toDate())
        val result = if (upperCaseFirstLetter) {
            "${dateFormatted[0].toUpperCase()}${dateFormatted.substring(1).toLowerCase()}"
        } else {
            dateFormatted
        }
